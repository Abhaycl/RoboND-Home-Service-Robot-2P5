...
// This function will accept robot movements and actuate the robot's wheels accordingly keep a low speed for better results
bool robot_move(const ROBOT_MOVEMENT move_type) {
    ...
    //Publish motor commands to the turtlebot and wait 10ms
...
// The laser callback function will be called each time a laser scan data is received
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg) {
    ...
    // Check if the turtlebot has crashed into a wall
    ...
    // Assign movements to a turtlebot that still did not crash 
    if (!crashed) {
        if (range_min <= 0.5 && !thats_a_door) {
            ...
            if (thats_a_door) {
                ...
                ROS_INFO("[ROBOT] I am going right: %d \n", thats_a_door);
            }
            else {
                robot_move(FORWARD);
            }
    ...
    // Turtlebot should go backward since it crashed into a wall
    else {
        robot_move(BACKWARD);
    }
}
...
int main(int argc, char** argv) {
    ...
    // Enter an infinite loop where the laser callback function will be called when new laser messages arrive
